...
void fix_ghosting_column(
    ...
    // Then exactly same procedure on the right side
    for (short i = 0; i<3; i++) {
        fix_ghosting_instance(matrix, i, possible_error_cause<<6, ((i+1)%3), possible_error<<6, error_fix<<6);
        fix_ghosting_instance(matrix, i, possible_error_cause<<6, ((i+2)%3), possible_error<<6, error_fix<<6);
    }
}
